...
. "$SCRIPTS_DIR/utils/remote/add_root_key.sh" "$DEVICE_IP" "$DEVICE_PORT" "$DEVICE_USER" "$DEVICE_PASS" || true
. "$SCRIPTS_DIR/utils/remote/refresh.sh" "$DEVICE_IP" "$DEVICE_PORT" "$DEVICE_USER" "$DEVICE_PASS" "$CHANNEL" "$CORE_CHANNEL" "$SNAPD_CHANNEL"
