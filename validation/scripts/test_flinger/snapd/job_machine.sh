...
test_data:
    test_cmds: |
        ...
        $PRE_HOOK
        . $JOBS_PROJECT/validation/scripts/utils/add_test_user.sh "$DEVICE_IP" "$DEVICE_PORT" "$DEVICE_USER" "generic" "ubuntu" "$TEST_USER_TYPE"
        ...
        . $JOBS_PROJECT/validation/scripts/utils/refresh.sh "$DEVICE_IP" "$DEVICE_PORT" "$DEVICE_USER" "" "$CHANNEL" "$CORE_CHANNEL" "$SNAPD_CHANNEL" || true
