...
test_data:
    test_cmds: |
        ...
        ssh ${DEVICE_USER}@${DEVICE_IP} "$JOBS_PROJECT/validation/scripts/utils/get-project.sh \"$CCONF_URL\" \"$PROJECT\" \"$BRANCH\" ''"
        ...
        ssh ${DEVICE_USER}@${DEVICE_IP} "$JOBS_PROJECT/validation/scripts/utils/run-spread.sh \"$HOST\" \"$PORT\" \"$PROJECT/tests/console-conf-tests\" \"$SPREAD_TESTS\" \"$SPREAD_ENV\" \"$SKIP_TESTS\" \"$SPREAD_PARAMS\""
EOF
