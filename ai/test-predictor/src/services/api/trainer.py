...
@trainer_bp.route('/retrain', methods=['POST'])
def manual_retrain():
    """Proxies the retraining request to the standalone Trainer Service."""
    try:
        # We use a very short timeout (1s) because we want the 
        # internal service to handle the 'background' part.
        logger.info("Forwarding manual retraining request to internal service...")
        
        # We don't wait for the whole retraining (which takes minutes)
        # The internal service should return 200/202 immediately or 429 if busy
        resp = requests.post(f"{TRAINER_INTERNAL_URL}/retrain", timeout=2)
        
        if resp.status_code == 200:
            return jsonify({"status": "success", "message": "Retraining triggered"}), 202
        elif resp.status_code == 429:
            return jsonify({"status": "error", "message": "Retraining already in progress"}), 429
        else:
            return jsonify({"status": "error", "message": "Internal trainer error"}), 500
        ...
        # If the trainer starts immediately, it might not respond in 2s
        # In this specific architecture, a timeout often means it started!
        return jsonify({"status": "success", "message": "Retraining initiated (ack)"}), 202
    except Exception as e:
        logger.error(f"Failed to reach internal trainer: {e}")
        return jsonify({"status": "error", "message": "Trainer service unreachable"}), 503
